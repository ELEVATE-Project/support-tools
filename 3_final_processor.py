...
REFINE_BATCH_SIZE = int(os.getenv("REFINE_BATCH_SIZE", "250"))
...
def generate_report():
    ...
    # Calculate PRI & School Representative metrics
    df_raw['pri_present'] = df_raw['PRI Member Information'].apply(
        lambda x: pd.notna(x) and str(x).strip().lower() not in ['', 'nan', 'none', 'n/a']
    )
    df_raw['school_rep_present'] = df_raw['School Representative Information'].apply(
        lambda x: pd.notna(x) and str(x).strip().lower() not in ['', 'nan', 'none', 'n/a']
    )
    pri_total_count = int(df_raw['pri_present'].sum())
    school_rep_total_count = int(df_raw['school_rep_present'].sum())
    pri_total_perc = (pri_total_count / TOTAL_CH_STATE * 100) if TOTAL_CH_STATE > 0 else 0
    school_rep_total_perc = (school_rep_total_count / TOTAL_CH_STATE * 100) if TOTAL_CH_STATE > 0 else 0
    ...
    def add_row_v3(t, metric, count, p_type, custom_pct=None):
        r = t.add_row().cells
        r[0].text = metric
        r[1].text = f"{int(count):,}" if isinstance(count, (int, float)) else str(count)
        if custom_pct is not None:
            r[2].text = f"{custom_pct:.1f}%"
        elif p_type == "none": r[2].text = "-"
    ...
    add_row_v3(table1, "Chaupals with PRI Participation", pri_total_count, "none", custom_pct=pri_total_perc)
    add_row_v3(table1, "Chaupals with School Representation", school_rep_total_count, "none", custom_pct=school_rep_total_perc)

    # SECTION 2.3: DISTRICT DISTRIBUTION
    doc.add_heading('2.3 District-wise Distribution of Reported Chaupals', level=2)
    dist_stats = df_raw.groupby('District').agg(
        Ch_Count=('id', 'count'), 
        Part_Sum=('Participant Count', 'sum'),
        Pri_Count=('pri_present', 'sum'),
        School_Count=('school_rep_present', 'sum')
    ).reset_index()
    dist_stats['Ch_Perc'] = (dist_stats['Ch_Count'] / TOTAL_CH_STATE) * 100
    dist_stats['Pri_Perc'] = (dist_stats['Pri_Count'] / dist_stats['Ch_Count']) * 100
    dist_stats['School_Perc'] = (dist_stats['School_Count'] / dist_stats['Ch_Count']) * 100
    dist_stats = dist_stats.sort_values('Ch_Count', ascending=False)

    table_dist = doc.add_table(rows=1, cols=9)
    table_dist.style = 'Table Grid'
    d_hdr = table_dist.rows[0].cells
    headers = [
        'District', 'Chaupals (N)', 'Chaupal %', 'Participants (N)', 'Participant %',
        'Chaupals with PRI (N)', 'PRI %', 'Chaupals with School Rep (N)', 'School Rep %'
    ]
    ...
    for _, row in dist_stats.iterrows():
        ...
        r[1].text = f"{int(row['Ch_Count']):,}"
        r[2].text = f"{row['Ch_Perc']:.1f}%"
        r[3].text = f"{int(row['Part_Sum']):,}"
        r[4].text = f"{(row['Part_Sum']/TOTAL_PART_STATE)*100:.1f}%"
        r[5].text = f"{int(row['Pri_Count']):,}"
        r[6].text = f"{row['Pri_Perc']:.1f}%"
        r[7].text = f"{int(row['School_Count']):,}"
        r[8].text = f"{row['School_Perc']:.1f}%"
